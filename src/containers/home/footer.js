...
const CollapseWrapper = ({ title="genres", items=[
    { label: "Mystery", url: "/" },
    { label: "Sci-Fi", url: "/" },
    { label: "Horror", url: "/" },
    { label: "Romance", url: "/" }
      ...
      <Collapse isOpen={isOpen}>
        {
          items.map((item, index) => (
            <NavLink key={index} className="nav-link p-0 my-1 text-white" to={item.url}>{item.label}</NavLink>
          ))
        }
      </Collapse>
    </div>
    ...
    <Container className="homepage-footer mt-5" fluid>
      <div className="d-md-none">
        ...
        <Row className="mt-3 text-white links-item">
          An experiential and interactive multi-platform streaming app and distribution channel dedicated to independently produced film and TV.
        </Row>
        <Row className="mt-3">
          <Button className="mr-3" shape="circle" icon={<SearchOutlined />} />
          <Button className="mr-3" shape="circle" icon={<SearchOutlined />} />
        ...
        <Row className="mt-3 pb-5 text-white">
          <Col className="p-0 font-normal" xs={8}>
            ...
            <div className="links-item">2021 © Indie Sparxs, All rights reserved.</div>
          </Col>
                ...
                <Collapse isOpen={languageCollapseOpen}>
                  {
                    languages.map((language, index) =>(
                      <NavLink key={index} className="nav-link p-0 my-1 text-white">{ language }</NavLink>
                    ))
                  }
                </Collapse>
              </div>
              ...
              <Col className="links" key={itemIndex}>
                <div className="links-title mb-3">{ title }</div>
                {
                  items.map(({ label }, index) => (
                    <div className="links-item my-2" key={index}>{ label }</div>
                  ))
                }
              </Col>
            ))
